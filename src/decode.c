...
dwg_decode_data(Bit_Chain * dat, Dwg_Data * dwg)
{
  ...
  PRE(R_2000)
    {
      LOG_INFO("WARNING: This version of LibreDWG is only capable "
               "of safely decoding version R2000 (code: AC1015) "
               "dwg-files.\n This file's version code is: %s Support "
               "for this version is still experimental. It might "
               "crash or give you invalid output.\n", version)
  ...
  VERSION(R_2004)
    {
      LOG_INFO("WARNING: This version of LibreDWG is only capable "
               "of properly decoding version R2000 (code: AC1015) "
               "dwg-files.\n This file's version code is: %s\n This "
               "version is not yet actively developed. It will "
               "probably crash and/or give you invalid output.\n",
               version)
  ...
  if (ent->picture_exists)
    {
      ...
      if (ent->picture_size < 210210)
        ...
        {
          LOG_ERROR("dwg_decode_entity: Absurd! Picture-size: "
                    "%lu kB. Object: %lu (handle).\n",
  ...
  if (bit_read_H(dat, &ref->handleref))
    {
      if (obj)
        {
          LOG_ERROR("Could not read handleref in object whose "
                    "handle is: %d.%d.%lu\n", obj->handle.code,
                    obj->handle.size, obj->handle.value)
        }
      else
        {
          LOG_ERROR("Could not read handleref in the header "
                    "variables section \n")
        }
      free(ref);
      return 0;
